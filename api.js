...
const getRandomAvatar = (seed) => `https://api.dicebear.com/7.x/initials/svg?seed=${seed}`;
...
// Storage for videos, comments and likes - this will reset on page refresh since it's not persistent
const videoStorage = new Map();

// Mock API function to get videos
async function getVideos(searchQuery) {
  // In a real application, this would be a fetch to a server
  // Simulate API delay
...
async function addComment(videoId, comment) {
  // In a real application, this would be a POST to a server
  // Simulate API delay
...
async function likeVideo(videoId, isLike) {
  // In a real application, this would be a POST to a server
  // Simulate API delay
